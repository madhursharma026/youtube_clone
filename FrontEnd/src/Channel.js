...
function Channel() {
    ...
    useEffect(() => {
        fetch(`http://127.0.0.1:5000/channel_details/${user.id}`).then((result) => {
            result.json().then((resp) => {
                setUserDetails(resp)
            })
        })
        fetch(`http://127.0.0.1:5000/channel_video_details/${user.id}`).then((result) => {
            result.json().then((resp) => {
                setUserVideos(resp)
                setloading(true)
            })
    ...
    return (
        <div className="Homepage">
            <title>{user.Channel_name}- Youtube</title>
            <Header />
                                                    ...
                                                    <Link style={{ textDecoration: "none" }} to={{ pathname: `/single_video/${UserVideos.video_id}`, id: UserVideos.video_id, }} onClick={setSingleVideoId(UserVideos.video_id)}>
                                                        ...
                                                        <div className="row mt-2">
                                                            ...
                                                            <div className="col-10">
                                                                ...
                                                                <p className='channel_video_date_views' style={{ fontSize: "15px", marginTop: "-18px" }}>{UserVideos.views} views • {UserVideos.upload_date}</p>
                                                            </div>
                                                        </div>
                                                    </Link>
                                                </div>
